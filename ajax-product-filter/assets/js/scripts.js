ttcirclebgcolor = function(){
    jQuery(".wcapf-layered-nav ul li a .tt-square").css('background', function () {
        return jQuery(this).closest('a').data('color')
    });
}
jQuery(document).ready(function($) {
	...
	wcapfFilterProducts = function() {
		...
		jQuery(".tt-archiver-wrapper .wcapf-before-products .products .product").hover(function(){
				var findName = jQuery(this).find(".woocommerce-loop-product__title").text();
				jQuery(this).attr('title', findName);
			});
		});
	}
    ...
    $(document).on('click', '.wcapf-active-filters a.reset', function(event) {
    	...
    	$('#wcapf-noui-slider .noUi-base .noUi-origin:first-child').css("left", "0")
        $('#wcapf-noui-slider .noUi-base .noUi-origin:last-child').css("left", "100%");
        $('#wcapf-noui-slider-value-min').text('$'+ min_val);
        $('#wcapf-noui-slider-value-max').text('$'+ max_val);
